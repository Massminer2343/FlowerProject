		...
		for(int i=0; i<canvas.length; i++)
		{
			for(int j=0; j<canvas[0].length; j++)
			{
				if(b.getSource()==canvas[i][j])
				{
					...
					if(fightSlection==3&&fireUse>0){
						damage=25;
						monsterHealth=monsterHealth-(fireReturns.diminishing_returns(damage,fireUse)*damageMod);
						fireUse=fireUse-5;
						damageMod=1;
						JOptionPane.showMessageDialog(null,"Fire type atacks are super effective against plant types");}
					else if(fightSlection==3&&fireUse<=0){
						JOptionPane.showMessageDialog(null,"OMFG THE FLANT IS IMMUNE TO FIRE NO DAMMAGE!!!!!");}
					...
					if(fightSlection==6){
						...
						JOptionPane.showMessageDialog(null,"10 damage not much but all atatck do more damage");
					}
